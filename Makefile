...
else ifeq ($(filter host-test-power-gate-policy host-test-task3-source-contract host-test-task4-source-contract host-test-task4-status-indicator-compile host-test-task4-neopixel-encoding host-test-task5-status-indicator-behavior host-test-task5-source-contract host-test-task6-source-contract,$(MAKECMDGOALS)),)
...
TASK5_STATUS_INDICATOR_BEHAVIOR_TEST = _build/host-tests/task5-status-indicator-behavior/status_indicator_behavior_test$(HOST_EXE_EXT)

host-test-power-gate-policy: $(POWER_GATE_POLICY_TEST)
	$(POWER_GATE_POLICY_TEST)

host-test-task3-source-contract:
	sh tests/task3-source-contract/task3_source_contract.sh

host-test-task4-source-contract:
	sh tests/task4-source-contract/task4_source_contract.sh
...
host-test-task6-source-contract:
	sh tests/task6-source-contract/task6_source_contract.sh
...
$(TASK5_STATUS_INDICATOR_BEHAVIOR_TEST): src/boards/k380/status_indicator.c src/boards/k380/status_indicator.h tests/task5-status-indicator-behavior/boards.h tests/task5-status-indicator-behavior/status_indicator_behavior_test.c
	@$(MKDIR) "_build/host-tests/task5-status-indicator-behavior"
	$(HOST_CC) -Wall -Wextra -Werror -Itests/task5-status-indicator-behavior -Isrc/boards/k380 -o $@ src/boards/k380/status_indicator.c tests/task5-status-indicator-behavior/status_indicator_behavior_test.c
