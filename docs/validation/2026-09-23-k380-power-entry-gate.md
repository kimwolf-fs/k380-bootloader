...
| 编号 | 操作 | 预期现象 | 实际结果 | 判定 |
| --- | --- | --- | --- | --- |
| H01 | 仅电池，运行 ZMK，按 `Fn+Del` | 三颗红灯各闪 3 次；不进入 bootloader；应用继续运行 | 待填写 | 待判定 |
| H02 | 仅电池，断电冷启动并按住 `Del` | 三颗红灯各闪 3 次；不出现蓝灯 bootloader 等待；进入应用 | 待填写 | 待判定 |
| H03 | USB 供电，运行 ZMK，按 `Fn+Del` | 按既有协议进入 bootloader，显示正常 bootloader 指示 | 待填写 | 待判定 |
